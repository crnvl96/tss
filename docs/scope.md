...
2. **Local infra**:
   ...
   - `pgvector/pgvector:pg18` Postgres+pgvector container, orchestrated via `docker compose` (see `docker-compose.yml` at the repo root; the `db:up` / `db:down` / `db:reset` / `db:logs` npm scripts wrap it)
